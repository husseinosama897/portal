<template>
   <div class="container-lg  border-container">
                            <div class="m-3  row">
                             ...
                             <div class="card">
                               <div class="card-body">
                                 <div class="card-header">        محاسبة الرواتب </div>
                                    <form method="POST" v-on:submit.prevent="submit()" enctype="multipart/form-data">
                                    <div class="row pt-4">
                                        <div class="col-lg-3 col-md-5 col-sm-5 col-xs-12">
                                            <label for="birthday">الموزع له </label>
                                            <p class="lightblue">الحساب الذي توزع له الأرباح </p>
   <input class="font-size" type="checkbox" value=""
                                            v-model="changer"     
...
<select class="form-select"  v-if="changer == true" disabled
>
<option></option>
</select>
                                               <span class="form-select" v-if="changer == false" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
...
{{accountt2.name}}
  ...
  <ul dir="rtl" class="dropdown-menu dropdown-menu-large" v-if="changer == false" style="height: 200px; width:100%;overflow-y:scroll; overflow-x: hidden;   text-align: start !important;
...
<a @click="updateaccount4(u)" v-if="u.sub_account3.length <= 0" class="dropdown-item" href="#">{{u.name}} - {{u.code}}</a>
                                        ...
                                        <div class="row pt-1">
                                            <div class="col-lg-3 col-md-5 col-sm-5 col-xs-12">
                                                 <label class="form-check-label" for="flexCheckDefault">
                                                    لن تدفع الرواتب الان </label>
                                                <input  type="checkbox" value="" class=" font-size"

  v-model="changer2"     
                                                 @click="changer2 != changer2"

                                                    id="flexCheckDefault">

                                            </div>
                                            <div class="col-lg-4 col-md-5 col-sm-5 col-xs-12 mt-2">
 <select class="form-select"  v-if="changer2 == true" disabled
>
<option></option>
</select>
                                                 <span class="form-select" v-if="changer2 == false" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
  <span class=" col-form-label col-form-label-sm text-end" >
{{accountt.name}}
  </span>
  ...
  <ul dir="rtl" v-if="changer2 == false" class="dropdown-menu dropdown-menu-large"  style="height: 200px; width:25%;overflow-y:scroll; overflow-x: hidden;   text-align: start !important;
  float: right !important;
  left: initial !important;
  right: 0 !important;
...
<a @click="updateaccount2(f)"  class="dropdown-item" href="#">{{f.name}} - {{f.code}}</a>
                                        ...
                                        <div class="row pt-4">
                                            ...
                                            <div class="col-lg-4 col-md-5 col-sm-5 col-xs-12 mt-2">
                                                <input type="text" class="form-control"  v-model="value" placeholder="القيمه"
                                                name="discription">
...
<script>
    export default {
     props:['easy_entry'],
  mounted() {
this.accountt2 = this.easy_entry.creditor_sub_account4 ?? this.easy_entry.creditor_sub_account3
this.accountt = this.easy_entry.debtor_sub_account3 ?? this.easy_entry.debtor_sub_account4
this.dis = this.easy_entry.dis
this.date = this.easy_entry.date
this.value = this.easy_entry.value
...
axios({
	...
	'url':'/managers/payrollaccountinga'
}).then(res=>{
...
axios({
	...
	'url':'/managers/moneymovementsaccount'
}).then(res=>{
...
if(this.changer2  == true ){
  this.accountt = {}
}


axios({
  url:'/managers/editeasymovement/'+this.easy_entry.id,
   method:'post',
   data:{
