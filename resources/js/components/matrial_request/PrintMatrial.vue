...
{{data2.ref}}
       ...
       <span class="t-points">  : </span>
            <div class="col-8">
                <span class="text-decoration-underline fw-md" v-if="data.project">{{data2.project.name }}</span>
            </div>
          </div>
    </div>
 ...
 {{data2.content}}
        ...
        <table class="table table-bordered border-primary mb-2">
            ...
            <tr  v-for=" (product,index) in  data2.attributes" :key="product.id">
               ...
               <td class="text-center fs-6">{{product.name ? product.name : product.dis }}</td>
                  ...
                  <td class="text-center fs-6">{{product.total ? product.total :  product.unit_price * product.qty }}</td>
                </tr>
    ...
    <div class="endsub mt-1 mb-1">
                <h5 class="mt-3 text-decoration-underline fw-md"> Matrials Condition :</h5>
  
   
        <p  v-for=" note in data2.note" :key="note.id" class="ms-5">{{note.dis}}</p>
 ...
 <div class="  row"
        >
                ...
                <div :key="data.id" class="col-2"  v-for="data in data.matrial_request_cycle" >
                    
                           <template v-if="data.comment_matrial_cycle  && data.status == 1">
                              <div class="col-2">
          <img :src="'/uploads/sign/'+data.comment_matrial_cycle.user.sign" alt="" width="80">
                           </div>
                           </template>

                              <template v-if="data.comment_matrial_cycle  && data.status == 2">
                              <div class="col-2">
...
this.payment.splice(index,1)
 ...
 this.data2 = this.data
          ...
          this.data2 =  this.$cookies.get('matrial_request');
