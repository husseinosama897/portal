                                        ...
                                        <tbody>
                                            <tr v-for="data in datas.data" :key="data.id">
                                                ...
                                                <td data-table="DESCRIPTION">{{data.subject}}</td>
                                                         <td data-table="STATUS" v-if="data.status == 2"><button type="button"  @click="syc(data.employee_cycle)" class="badge bg-danger border-0" data-toggle="modal" data-target="#hhh"> REJECTED</button></td>
                                                                 <td data-table="STATUS"  v-if="data.status == 1"><button type="button"  @click="syc(data.employee_cycle)" class="badge bg-success border-0" data-toggle="modal" data-target="#hhh">ACCEPTED</button></td>
                                                                 <td data-table="STATUS" v-if="data.status == 0"><button type="button"   @click="syc(data.employee_cycle)" class="badge bg-warning border-0" data-toggle="modal" data-target="#hhh">BENDING</button></td>
...
<div class="dropdown">
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
    <a class="dropdown-item" :href="'/user/employeereturn/'+data.id">preview</a>
...
real_cycle:[
  ...
  {step:2,name:"Projects Director",class:3},
  {step:3,name:'General Manager',class:4},
],
...
this.datas.data.splice(index,1)
    ...
    getResults(page = 1) {
		axios({
  method: 'get',
  url: '/user/returnemployeeasjson?page=' + page,
