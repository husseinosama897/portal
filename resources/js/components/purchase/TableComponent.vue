                     ...
                     <tbody>
                        <tr v-for="data in datas.data" :key="data.id">
                           ...
                           <td data-table="STATUS" v-if="data.status == 2">
                              <button type="button" @click="syc(data.purchase_order_cycle)"  class="badge bg-danger border-0" data-toggle="modal" data-target="#hhh">
                              REJECTED
                              </button>
                           </td>
                           <td data-table="STATUS"  v-if="data.status == 1">
                              <button type="button" @click="syc(data.purchase_order_cycle)" class="badge bg-success border-0" data-toggle="modal" data-target="#hhh">
                              ACCEPTED
                              </button>
                           </td>
                           <td data-table="STATUS" v-if="data.status == 0">
                              <button type="button"   @click="syc(data.purchase_order_cycle)"  class="badge bg-warning border-0" data-toggle="modal" data-target="#hhh">
                              BENDING
                              </button>
                           </td>
                           ...
                           <td  data-table="ACTION">
                              <div class="dropdown">
                                 ...
                                 <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                    <a class="dropdown-item" :href="'/user/purchasereturn/'+data.id">preview</a>
                                    <a class="dropdown-item" :href="'/user/purchase_update/'+data.id">update</a>
...
export default {
	...
	methods: {
		...
		getResults(page = 1) {
			axios({
				method: "get",
				url: "/user/returnasjson?page=" + page
			}).then(response => {
