<template>
        ...
        <div>
            	<button @click="print()" type="button" class="btn btn-light">
        ...
        <table class="table table-bordered border-primary mb-2">
            <thead>
                <tr class="fw-md">
                    ...
                    <th scope="col" class="fw-md fs-6">previous_qty</th>
                

                    <th scope="col" class="fw-md fs-6">total_qty</th>
              ...
              <tbody>
                ...
                <td class="text-center fs-6">{{product.name ? product.name : product.dis }}</td>
                  ...
                  <td class="text-center fs-6">{{product.previous_qty}}</td>

                  <td class="text-center fs-6">{{product.total_qty}}</td>
                  <td class="text-center fs-6">{{product.Excution_percentage}}  %</td>
                  ...
                  <td class="text-center fs-6">{{product.total}}</td>
                   ...
                   <tr>
                    <td colspan="8" class="text-center fs-6"> VAT</td>
                    <td class="text-center fs-6">{{data2.vat}}</td>
                </tr>
                
                <tr>
                    <td colspan="8" class="text-center fs-6">Total Cost</td>
 ...
 <div class="qr row pt-4 pb-4">
        <div class="col-6">
          <qrcode-vue :value="this.route" size="90" level="L" />
           ...
           <div :key="data.id"  v-for="data in data.subcontractor" >
                                            <template v-if="data.comment_subcontractor_cycle && data.status == 1">
                                               <div class="col-2" >
                      <img :src="'/uploads/sign/'+data.comment_subcontractor_cycle.user.sign" alt="" width="80">
                                               </div>
                                         </template>

                                          <template v-if="data.comment_subcontractor_cycle && data.status == 2">
                                               <div class="col-2" >
...
var total_qty = (Number(e.currentqty) + Number(e.previous_qty))
                  data.push({name: e.name ? e.name : e.dis ,  qty:e.qty, unit:e.unit,previous_qty:e.previous_qty, 
