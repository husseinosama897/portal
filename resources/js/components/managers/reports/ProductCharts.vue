...
updatechart(){
 ...
 this.data.forEach(e=>{
   ...
   e.offerpaid.forEach(offerpaid => {

     this.series.forEach(z=>{
       if(z.name == e.name){
         z.data.push( [ moment(offerpaid.date).format(" D  MMM  YY"), e.offerpaid_count])
         current = 1
         console.log(1)
       }
         })
...
data:[
    ...
    [ moment(offerpaid.date).format(" D  MMM  YY"), e.offerpaid_count]
]
})
