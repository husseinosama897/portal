                  ...
                  <table id="basic-datatable" class="table dt-responsive nowrap w-100">
                     ...
                     <tbody>
                        <tr v-for="data in datas.data" :key="data.id">
                           ...
                           <td data-table="STATUS" v-if="data.status == 2"><button type="button"  @click="syc(data.site_cycle)" class="badge bg-danger border-0" data-toggle="modal" data-target="#hhh"> REJECTED</button></td>
                           <td data-table="STATUS"  v-if="data.status == 1"><button type="button"  @click="syc(data.site_cycle)" class="badge bg-success border-0" data-toggle="modal" data-target="#hhh">ACCEPTED</button></td>
                           <td data-table="STATUS" v-if="data.status == 0"><button type="button"   @click="syc(data.site_cycle)"  class="badge bg-warning border-0" data-toggle="modal" data-target="#hhh">BENDING</button></td>
                           <td  data-table="ACTION">
                              <div class="dropdown">
                                 ...
                                 <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                    <a class="dropdown-item" :href="'/user/sitereturn/'+data.id">preview</a>
...
axios({
    url:'/user/delete_site_data/'+data.id,
    method:'post',
}).then(res=>{
this.datas.data.splice(index,1)
    ...
    getResults(page = 1) {
		axios({
  method: 'get',
  url: '/user/returnsiteasjson?page=' + page,
