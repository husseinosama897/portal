...
this.$cookies.set('site',data);


 window.location.href = ('/user/presitereturn')
...
if(this.userz.length > 0){
     ...
     axios.post('/user/siteinsrting',formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data' },
})
.then(res=>{
  this.$cookies.remove("site");

 window.location.href = ('/user/index_site_inv')
...
this.payment.splice(index,1)
        ...
        mounted() {
           this.project()
                             var dataa =  this.$cookies.get('site');
