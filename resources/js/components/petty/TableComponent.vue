                                        ...
                                        <tbody>
                                            <tr v-for="data in datas.data" :key="data.id">
                                                ...
                                                <td data-table="DESCRIPTION">{{data.subject}}</td>
                                                 <td data-table="STATUS" v-if="data.status == 2"><button type="button"  @click="syc(data.petty_cash_cycle)" class="badge bg-danger border-0" data-toggle="modal" data-target="#hhh"> REJECTED</button></td>
                                                                 <td data-table="STATUS"  v-if="data.status == 1"><button type="button"  @click="syc(data.petty_cash_cycle)" class="badge bg-success border-0" data-toggle="modal" data-target="#hhh">ACCEPTED</button></td>
                                                                 <td data-table="STATUS" v-if="data.status == 0"><button type="button"   @click="syc(data.petty_cash_cycle)"  class="badge bg-warning border-0" data-toggle="modal" data-target="#hhh">BENDING</button></td>
...
<div class="dropdown">
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
    <a class="dropdown-item" :href="'/user/petty_cashreturn/'+data.id">preview</a>
    <a class="dropdown-item" @click="dele(data,index)"  href="#">delete</a>
    <a class="dropdown-item" :href="'/user/petty_cash_edit/'+data.id">edit</a>
  </div>
</div>
												</td>
                                            </tr>
...
axios({
    url:'/user/delete_petty_cash_data/'+data.id,
    method:'post',
}).then(res=>{
this.datas.data.splice(index,1)
    ...
    getResults(page = 1) {
		axios({
  method: 'get',
  url: '/user/returnpetty_cashasjson?page=' + page,
    ...
    datajson() {
		axios({
  method: 'get',
  url: '/user/returnpetty_cashasjson/' ,
