...
this.$cookies.set('frq',data);


 window.location.href = ('/user/prefrqreturn')
...
if(this.userz.length > 0){
     ...
     axios.post('/user/frqinsrting',formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data' },
})
.then(res=>{
 
this.$cookies.remove("frq");
          window.location.href = ('/user/index_frq_inv')
...
this.payment.splice(index,1)
        ...
        mounted() {
           this.project()
                     var dataa =  this.$cookies.get('frq');
